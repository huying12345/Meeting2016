...
//     如果想修改此文件的方法，请在此文件的上级目录中的DocInformationViewController重写(override)该方法。
...
namespace Yamon.Module.Doc.WebApi
{
    ...
    /// 公文视图
    /// </summary>
    public partial class _DocInformationViewController :BaseController
    {

        public DocInformationViewDAL dal = new DocInformationViewDAL();
        ...
        [ValidateInput(false)]
        [LogFilter]
        public virtual ActionResult Create(DocInformationView model,bool returnData=false)
        {
            object newId = "";
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    hash["id"] = newId;
                    if(returnData){
                        DocInformationView newrow = dal.GetModelByID(newId);
                        hash["data"] = newrow;
                    }
                }
        ...
        [LogFilter]
        public virtual ActionResult Edit(DocInformationView model,bool returnData=false)
        {
            try
            {
               ...
               if (result > 0)
                {
                    hash["id"] = model.InfoID;
                    if(returnData){
                        DocInformationView newrow = dal.GetModelByID(model.InfoID);
                        hash["data"] = newrow;
                    }
                }
            }
        ...
        [NonAction]        
        public  virtual List<DocInformationView> GetGridList(string filterId, int showValue, out int totalRows)
        {
            totalRows = 0;
            ...
            List<DocInformationView> modelList = new List<DocInformationView>();
            switch(filterId)
            {
        ...
        public  virtual ActionResult GridByFilterID(string filterId)
        {
            ...
            catch (Exception ex)
            {
                hash["success"] = false;
                hash["message"] = ex.Message;
                hash["rows"] = new List<DocInformationView>();
            }
            return Content(JsonConvert.SerializeObject(hash)); 
        }
    ...
    public virtual ActionResult Delete(string id)
    {
        ...
        [CheckPurview(pageCode:"Doc_DocInformationView_Show")]        
        public ActionResult GetEntityByID(string id)
        {
            DocInformationView model = dal.GetModelByID(id);
            if (model != null)
            {
                hash["data"] = model;
                hash["success"] = true;
